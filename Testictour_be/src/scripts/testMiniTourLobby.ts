...
async function triggerSyncAndVerify(lobbyId: string) {
  ...
  try {
    ...
    const targetParticipantsPuids = lobby.participants.map((p: any) => p.user.puuid!);
      ...
      const newMiniTourMatch = await prisma.miniTourMatch.create({
        data: {
          ...
          miniTourMatchResults: {
            create: lobby.participants.map((p: any) => ({
              userId: p.userId,
              placement: 0,
              points: 0,
            })),
...
if (require.main === module) { main(); } else { module.exports = { seedMiniTourLobby: main }; } 
