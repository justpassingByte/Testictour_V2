...
async function main() {
  ...
  // --- Bổ sung: Tạo lobby và thêm 8 người dùng vào ---
  console.log(`\nBắt đầu tạo lobby và thêm 8 người chơi...`);
  ...
  // 2. Lấy thông tin 8 người dùng (từ PlayerOne đến PlayerEight)
  const usersToAddToLobby = await prisma.user.findMany({
    where: {
      username: {
        in: ['PlayerOne', 'PlayerTwo', 'PlayerThree', 'PlayerFour', 'PlayerFive', 'PlayerSix', 'PlayerSeven', 'PlayerEight']
      }
    }
  });
  
  // 3. Thêm 8 người dùng vào lobby và cập nhật số dư, prize pool trong một giao dịch
  try {
