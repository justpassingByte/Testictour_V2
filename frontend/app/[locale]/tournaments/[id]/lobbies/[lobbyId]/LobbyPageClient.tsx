            ...
            <Tabs defaultValue="overview" className="w-full">
              ...
              <TabsContent value="matches" className="space-y-4 pt-4">
                <div className="space-y-3">
                  {Array.from({ length: ['swiss', 'snake'].includes(liveLobbyData?.round?.phase?.type?.toLowerCase()) ? Math.max(1, liveLobbyData?.matches?.length || 0) : Math.max(liveLobbyData?.round?.phase?.matchesPerRound || 1, liveLobbyData?.matches?.length || 0) }).map((_, idx: number) => {
