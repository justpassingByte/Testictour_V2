    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
        <Card className="bg-gradient-to-br from-blue-500/10 to-blue-600/5 border-blue-500/20">
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-1">
              <span className="text-[11px] text-blue-400 font-semibold uppercase tracking-wide">Players</span>
              <Users className="h-3.5 w-3.5 text-blue-400" />
            </div>
            <p className="text-2xl font-bold">{allPlayers.length}</p>
            <div className="flex gap-1.5 mt-1.5 flex-wrap">
              <Badge variant="outline" className="text-[9px] px-1.5 py-0 bg-green-500/10 text-green-400 border-green-500/20">{playersAdvanced} adv</Badge>
              <Badge variant="outline" className="text-[9px] px-1.5 py-0 bg-red-500/10 text-red-400 border-red-500/20">{playersEliminated} out</Badge>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-emerald-500/10 to-emerald-600/5 border-emerald-500/20">
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-1">
              <span className="text-[11px] text-emerald-400 font-semibold uppercase tracking-wide">Avg Points</span>
              <TrendingUp className="h-3.5 w-3.5 text-emerald-400" />
            </div>
            <p className="text-2xl font-bold text-emerald-400">{avgPoints.toFixed(1)}</p>
            <p className="text-[10px] text-muted-foreground mt-1">{totalPointsAwarded} pts total</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-yellow-500/10 to-yellow-600/5 border-yellow-500/20">
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-1">
              <span className="text-[11px] text-yellow-400 font-semibold uppercase tracking-wide">Top Score</span>
        ...
        <Card className="bg-gradient-to-br from-violet-500/10 to-violet-600/5 border-violet-500/20">
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-1">
              <span className="text-[11px] text-violet-400 font-semibold uppercase tracking-wide">Regions</span>
              <Globe className="h-3.5 w-3.5 text-violet-400" />
            </div>
            <p className="text-2xl font-bold">{regionSet.length}</p>
            <p className="text-[10px] text-muted-foreground mt-1">{regionStats[0]?.region ?? "—"} leads</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Card className="bg-card/60 dark:bg-card/40 backdrop-blur-lg border border-white/20">
          <CardHeader className="pb-2">
            ...
            <CardDescription className="text-xs">How often each placement occurred across all matches</CardDescription>
          </CardHeader>
          <CardContent>
            {placementCounts.every(p => p.count === 0) ? (
              <p className="text-xs text-muted-foreground text-center py-8">No match data yet</p>
            ) : (
              <MiniBar
        ...
        <Card className="bg-card/60 dark:bg-card/40 backdrop-blur-lg border border-white/20">
          <CardHeader className="pb-2">
            ...
            <CardDescription className="text-xs">Top player scores in this round</CardDescription>
          </CardHeader>
          <CardContent>
            {topByPoints.length === 0 ? (
              <p className="text-xs text-muted-foreground text-center py-8">No points data yet</p>
            ) : (
              <MiniBar
      ...
      {regionStats.length > 0 && (
        <Card className="bg-card/60 dark:bg-card/40 backdrop-blur-lg border border-white/20">
          <CardHeader className="pb-2">
            ...
            <CardDescription className="text-xs">Average points and advancement rate by region</CardDescription>
          </CardHeader>
          <CardContent>
                  ...
                  <div key={r.region} className="space-y-1.5">
                    <div className="flex items-center justify-between text-sm">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-muted-foreground text-xs">{r.total} players</span>
                      </div>
                      <div className="flex items-center gap-3 text-xs">
                        <span className="font-medium">{r.avg.toFixed(1)} avg pts</span>
