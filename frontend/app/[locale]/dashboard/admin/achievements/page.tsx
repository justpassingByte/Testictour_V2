...
const MOCK_ACHIEVEMENTS: Achievement[] = [
  ...
  { id: "4", name: "Grandmaster", description: "Win 10 tournaments.", icon: "Crown", category: "performance", condition: "tournaments_won", conditionValue: 10, points: 500, rarity: "LEGENDARY", earnedCount: 12, createdAt: "2024-01-01" },
  { id: "5", name: "Veteran", description: "Play 50 matches.", icon: "Shield", category: "participation", condition: "matches_played", conditionValue: 50, points: 75, rarity: "RARE", earnedCount: 210, createdAt: "2024-01-01" },
]
...
export default function AdminAchievementsPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editTarget) {
        await api.put(`/admin/achievements/${editTarget.id}`, form)
        ...
        const res = await api.post('/admin/achievements', form).catch(() => null)
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.delete(`/admin/achievements/${deleteConfirm.id}`).catch(() => null)
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">{t("achievements_tab", { defaultValue: "Achievements" })}</h1>
          <p className="text-muted-foreground text-sm">{t("manage_achievements_desc", { defaultValue: "Create and manage player achievements." })}</p>
        </div>
