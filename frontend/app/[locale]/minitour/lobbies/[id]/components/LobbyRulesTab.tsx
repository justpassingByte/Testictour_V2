...
export function LobbyRulesTab({ lobby }: LobbyRulesTabProps) {
  const guideTips = lobby.settings?.guideTips
  const oldTips = typeof lobby.settings?.tips === "string" ? lobby.settings.tips : null
  const hasGuide = hasGuideTipsContent(guideTips) || Boolean(oldTips?.trim())

  return (
    <div className="space-y-4">
      {hasGuide && <GuideTipsRenderer guideTips={guideTips} oldTips={oldTips} />}

      <Card className="rounded-2xl border border-slate-700/80 bg-slate-950/60">
