  ...
  return (
    <div className="space-y-4">
      {tournaments.length === 0 ? (
        <p className="text-muted-foreground">{t("no_tournaments_found")}</p>
      ) : (
          ...
          <Card key={tournament.id} className="group transition-all hover:border-primary border border-gray-700/50 bg-card/60 dark:bg-card/40 backdrop-blur-lg transition-all duration-300 hover:shadow-lg hover:shadow-primary/10 hover:-translate-y-1">
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                >
                  {tournament.status === "ongoing" ? t("ongoing") : t("finished")}
                </Badge>
              </div>
              <p className="text-muted-foreground text-sm mb-2">
                {t("registered")}: {new Date(tournament.joinedAt).toISOString().split('T')[0]} • {t("round")} {tournament.currentRound} {t("of")} {tournament.totalRounds}
              </p>
              <div className="grid grid-cols-3 gap-4 text-sm mb-6">
                <div>
                  <p className="text-muted-foreground">{t("current_placement")}</p>
                ...
                <div>
                  ...
                  >
                    {tournament.eliminated ? t("eliminated") : t("active")}
                  </Badge>
                </div>
              </div>
              <Link href={`/tournaments/${tournament.id}`}>
                <Button variant="outline" className="w-full flex items-center group-hover:bg-primary group-hover:text-white">
                  {t("view_tournament")} <ChevronRight className="ml-2 h-4 w-4 transition-transform group-hover:translate-x-1" />
                </Button>
              </Link>
            </CardContent>
          </Card>
