            ...
            if (lobby.matches) {
              for (const match of lobby.matches) {
                if (match.matchResults && match.matchResults.length > 0) {
                  // Primary source: DB match results generated by backend
                  allMatchResults[match.id] = match.matchResults;
                } else if (match.matchData && 'info' in match.matchData && match.matchData.info?.participants) {
                  // Fallback for very old/legacy data formats where points were injected into info
                  const results: IMatchResult[] = match.matchData.info.participants.map((p: any) => ({
                    matchId: match.id,
